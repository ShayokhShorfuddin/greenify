...
export type Type_IPToCo2IntensityAPIResponse = {
  ...
  kind: "IPToCo2IntensityAPIResponse";
  year: number;
};
...
export async function IPToCo2IntensityCardContent({ url }: { url: string }) {
  ...
  if (getIpResult.errorOccurred) {
    return <p className="text-sm mt-1 text-red-500">{FailedToResolveIPText}</p>;
  }

  // If IP is obtained successfully, call the IP to CO2 intensity API
  let result: Type_IPToCo2IntensityAPIResponse;
  ...
  try {
    ...
    result = {
      ...apiResponse,
      kind: "IPToCo2IntensityAPIResponse",
    } as Type_IPToCo2IntensityAPIResponse;
  } catch (error) {
